  ...
  export const  UpdateUser = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if ( !user.email || !user.password || !user.first_name || !user.last_name) {
        res.status(400).json({ error:"Missing/Invalid parameters, the following parameters are required:  email, password, first_name, last_name" })
        return
      }
...
export const loginUser = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (user_Id) {
        ...
        res.status(200).json({ Login: 'Success', id: user_Id, token: user_Token })
      } else {
